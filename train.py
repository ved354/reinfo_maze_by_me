...
envirolmentes_in_list=[ envirolment(15,15,14,14,-1,-60,4000,25,100) for _ in range(10)]
gamma=0.7
learning_rate=0.01
path="checkpoint.pth"
...
def efficitency_cal(episodes):
    count=0
    for env in envirolmentes_in_list:
        count+=env.reach_target
...
def trainer(episodes_given,max_step,no_of_env):
    main_network=Network(15*15,40,20,4)
    agent_in_work=agent(main_network,learning_rate=0.0001,gamma=0.95)
...
#reset_path(path)
see=trainer(200,100,10)
see2=trainer(200,100,10)
see3=trainer(200,100,10)
see4=trainer(200,100,10)
print(efficitency_cal(800))



#it is not leanrnig from the histroy or it is learning wrong from the history ??
    #yes it learning from the history
#work on teh building of the wall make it a real walles
    #had a face where we will have col,roe walls but not yet fully randmised
        #but we need to enhanse it but by the rnadomisation
#as i oberved the probabulity of the 1 is getting incresed which is the problem over the time it is thinking down is good over time
    #it is due to my mistake which was caused due to teh reward system
###i think we have a problem in the loss function
    #didint inspect upto now
        #
#or we are not ubdting the weightes proparley---sol once see the compuational graph of it so we can understand in clear
    #found the problem but need to work on how to see on the compuational thinges realley in the graphes frace
